...
const { getPool, allTools, featuredTools, getTool, getCategories, getArticles, getArticle, insert, dashboardStats, adminList, upsertTool } = require("../lib/db");
...
router.post("/leads", asyncHandler(async (req, res) => res.status(201).json(await insert("leads", {
  first_name: clean(req.body.first_name),
  email: clean(req.body.email),
  answers: JSON.stringify(req.body.answers || {}),
  results: JSON.stringify(req.body.results || [])
}))));
...
router.post("/reviews", asyncHandler(async (req, res) => res.status(201).json(await insert("reviews", { tool_slug: clean(req.body.tool_slug), first_name: clean(req.body.first_name), email: clean(req.body.email), rating: Number(req.body.rating || 5), content: clean(req.body.content) }))));
...
router.post("/admin/login", asyncHandler(async (req, res) => {
  ...
  if (!db) {
    if (email === (process.env.ADMIN_EMAIL || "admin@crp.local") && password === (process.env.ADMIN_PASSWORD || "admin123")) {
