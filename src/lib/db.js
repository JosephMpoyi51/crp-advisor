...
async function adminList(table) { const db = getPool(); if (!db) return memory[table] || []; const [rows] = await db.execute(`SELECT * FROM ${table} ORDER BY id DESC LIMIT 200`); return rows; }
...
module.exports = { getPool, initSchema, allTools, featuredTools, getTool, getCategories, getArticles, getArticle, insert, dashboardStats, adminList, setReviewStatus, deleteRow, upsertTool };
