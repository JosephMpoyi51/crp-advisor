...
async function seedDefaults(db) {
  ...
  if (!toolRows[0].total) {
    ...
    for (const item of tools) await db.execute("UPDATE tools SET icon_url = :icon_url WHERE slug = :slug AND (icon_url IS NULL OR icon_url = '')", { slug: item.slug, icon_url: item.icon_url || "" });
  }
}
