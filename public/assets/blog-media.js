(() => {
  ...
  routes["/blog"] = blogIndexWithImages;
  const previousRender = render;
  render = async function mediaBlogRender() {
    if (location.pathname.startsWith("/blog/")) {
      setActiveNav("/blog");
      document.querySelector("#app").innerHTML = await articleWithImage(location.pathname.split("/").pop());
      bindCopyButtons();
      return;
    }
  ...
  async function blogIndexWithImages() {
  ...
  async function articleWithImage(slug) {
    ...
    return `<article class="page article-page"><p class="eyebrow">${escapeHtml(article.category || "Guide")}</p><h1>${escapeHtml(article.title)}</h1><p class="lead">${escapeHtml(article.excerpt || "")}</p>${articleHero(article)}<div class="article-meta"><span class="author-chip"><span class="author-avatar">${escapeHtml(article.author_initials || initials(article.author))}</span>${escapeHtml(article.author || "CRP Advisor")}</span><span>${formatDate(article.published_at)}</span><span>${Number(article.reading_minutes || readingMinutes(article.content))} min de lecture</span></div><div class="share-row"><a href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}" target="_blank" rel="noopener">Facebook</a><a href="https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${title}" target="_blank" rel="noopener">Twitter</a><a href="https://www.threads.net/intent/post?text=${title}%20${encodeURIComponent(url)}" target="_blank" rel="noopener">Threads</a><a href="mailto:?subject=${title}&body=${encodeURIComponent(url)}">Email</a><button type="button" data-copy-link="${escapeAttribute(url)}">Copier le lien</button></div><hr class="article-separator"><div class="article-content">${paragraphs(article.content).map((text) => `<p>${escapeHtml(text)}</p>`).join("")}</div><hr class="article-separator"><section class="article-section"><h2>Articles similaires</h2><div class="related-grid">${related.map(relatedCard).join("")}</div></section><hr class="article-separator"><section class="article-section"><p class="eyebrow">Tendance cette semaine</p><h2>Outils en vedette</h2><div class="weekly-tools">${weeklyTools.slice(0, 4).map(weeklyToolCard).join("")}</div></section></article>`;
  }

  function articleImage(article) { return article.featured_image_url || defaultImages[article.slug] || "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?auto=format&fit=crop&w=1400&q=80"; }
  function articleHero(article) { return `<figure class="article-hero-image"><img src="${escapeAttribute(articleImage(article))}" alt="${escapeAttribute(article.title)}" loading="lazy"></figure>`; }
  function articleCard(article) { return `<a class="blog-card blog-card-media" href="/blog/${article.slug}" data-link><img src="${escapeAttribute(articleImage(article))}" alt="" loading="lazy"><span class="tag">${escapeHtml(article.category || "Guide")}</span><h2>${escapeHtml(article.title)}</h2><p>${escapeHtml(articleExcerpt(article, 80, 95))}</p><div class="article-meta"><span>${escapeHtml(article.author || "CRP Advisor")}</span><span>${formatDate(article.published_at)}</span><span>${Number(article.reading_minutes || readingMinutes(article.content))} min</span></div></a>`; }
  function relatedCard(article) { return `<a class="related-card related-card-media" href="/blog/${article.slug}" data-link><img src="${escapeAttribute(articleImage(article))}" alt="" loading="lazy"><span class="tag">${escapeHtml(article.category || "Guide")}</span><h3>${escapeHtml(article.title)}</h3><p>${escapeHtml(articleExcerpt(article, 35, 55))}</p></a>`; }
  function weeklyToolCard(tool) { const icon = tool.icon_url ? `<div class="logo"><img src="${escapeAttribute(tool.icon_url)}" alt="" loading="lazy" onerror="this.remove()"></div>` : `<div class="logo">${escapeHtml(tool.name?.[0] || "C")}</div>`; return `<a class="weekly-tool" href="/outil/${tool.slug}" data-link>${icon}<h3>${escapeHtml(tool.name)}</h3><p>${escapeHtml(tool.description || "")}</p><span class="tag">${tool.editorial_score || 0}/100</span></a>`; }
  function bindCopyButtons() { document.querySelectorAll("[data-copy-link]").forEach((button) => { if (button.dataset.bound) return; button.dataset.bound = "1"; button.addEventListener("click", async () => { await navigator.clipboard?.writeText(button.dataset.copyLink).catch(() => null); button.textContent = "Lien copié"; }); }); }
